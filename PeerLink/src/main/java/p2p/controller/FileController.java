...
public class FileController {
    ...
    public FileController(int port) throws IOException {
        ...
        File uploadDirFile = new File(uploadDir);/*
                                                  * new File(uploadDir): This creates a File object that
                                                  * points to the directory specified by uploadDir. It does not create
                                                  * the directory on disk yet; it
        ...
        if (!uploadDirFile.exists()) { // uploadDirFile = objec that points to the "peerlink-uploads file"
            uploadDirFile.mkdirs();/*
                                    * This method creates the directory represented by uploadDirFile,
                                    * For example, if peerlink-uploads or any parent folder does not exist, it will
                                    * create them all.
                                    */

            /*
             * Example
             * Suppose uploadDir is
             * "C:\\Users\\John\\AppData\\Local\\Temp\\peerlink-uploads":
    ...
    private class CORSHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            /*
             * HttpExchange exchange: This object represents the HTTP request and response.
             * It contains:
             ** An HTTP Exchange has both the request and the response section all the time
             * in it
             * 
             ...
             * 
             * 
             * Here's a small EXAMPLE showing what data is available in an HttpExchange
             * object:
             * -----------------------
             * Method: POST
             * Path: /api/users
             * Headers: {
             * Authorization=[Bearer abc123token],
             * Content-type=[application/json], Host=[localhost:8080] ← This tells us what's
             * in the body
             * }
             * Request Body: {"name": "Alice", "email": "alice@example.com"} ← This IS the
             * JSON data
             * ---------------------------
             * The HttpExchange object is your gateway to everything about the HTTP request
             * coming in and
             ...
             * Host=[localhost:8080], Content-length=[1547]}
    ...
    private static class MultipartParser {
        ...
        private int findSequence(byte[] data, byte[] sequence, int startPos) {
            outer: for (int i = startPos; i <= data.length - sequence.length; i++) {
     ...
     * When someone sends a request to /upload, the server calls handle() in your
     * UploadHandler to process that request.
     */
    private class UploadHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange exchange) throws IOException {
            /*
             * Here's an example of HTTP Exchange objects header section in a
             * request:(Header is a section in whole HTTP Exchange object)
             * 
            ...
            headers.add("Access-Control-Allow-Origin", "*");/*
             * The "*" is like a wild card that means
             * "everyone is welcome!"
             * You could also be more specific:
             * 
             * "https://friendsite.com" =
             * "Only my friend's site can visit"
             * "*" = "Everyone can visit!"
             * 
             * So this one line of code is basically your server being
             * friendly and welcoming to visitors from other websites,
             * instead of being unfriendly and saying "go away!"
             * ---------------------------------------------------------
             * ---
             * Let's say you're on Google Search
             * (https://www.google.com) and you click on Google Maps:
             * What happens:
             * 
             ...
             * If Maps wants to share data with Search, it needs CORS
             * permission!
             * 
             ...
             * ---------------------------------------------------------
             * ---
             * Option 1: Allow EVERYONE (not recommended for sensitive
             * data)
             * headers.add("Access-Control-Allow-Origin", "*");//* =
             * everyone can access my data
             * This means:
             * "Any website from anywhere can access my data"
             * 
             ...
             * If someone knows the address (URL and port) of your server, any website can
             * send requests to upload or download files.
             * This is because your server is telling browsers: “I accept requests from
             * anywhere.”
             */
            ...
            if (!exchange.getRequestMethod().equalsIgnoreCase("POST")) {
                ...
                /*
                 * 1. exchange.getRequestMethod(): Gets the HTTP method (GET, POST, PUT, DELETE,
                 * etc.)
                 * 2. .equalsIgnoreCase("POST"): Checks if the method is "POST"
                 * (case-insensitive)
                 ...
                 * Parameter 2 (response.getBytes().length): The exact size of the response body
                 * in byt
                 * response.getBytes()
                 * 
                 ...
                 * 1.exchange.getResponseBody(): Gets the output stream where you write the
                 * response
                 ...
                 * What it means:
                 * "I understand your request, but I don't support that HTTP method"
                 * 
                 * Common scenario:
                 ...
                 */
                return;
                // HOW THIS CODE WORKS IN REAL WORLD EXAMPLE of the above code please check it
                // on claude notes as well:
                /*
             ...
             * 
             * Codes Result:(getFirst()-> it gets the very first thing of the kind mentioned
             * in the braces
            ...
            // This "if" cond: // Check if the request is correctly formatted
            // (multipart/form-data)
            if (contentType == null || !contentType.startsWith("multipart/form-data")) {
                 ...
                 * End Boundary
                 * ------WebKitFormBoundary1234--
                 * The double dashes -- at the end mean
                 * "this is the final boundary, no more parts coming"
                 * Simple Analogy
            ...
            try {
                 ...
                 * 
                 * IOUtils.copy(exchange.getRequestBody(), baos);->
                 * Reads all bytes from the request body stream and writes them to the
                 ...
                 * 
                 * What is parsing? Parsing is like taking apart a LEGO castle to see all the
                 * individual pieces!
                 * Imagine you have a big box of mixed-up LEGO pieces all jumbled together.
                 * Parsing is when you:
                 * 
                 ...
                 * 
                 * Example for a kid: Let's say you have a sentence written without spaces:
                 * "catdogbirdfish"
                 ...
                 * The Problem: When you upload a file on a website (like uploading a photo to
                 * Facebook),
                 * the computer receives everything mixed together in one big chunk, like this
                 * messy package:
                 * 
                 * ------BOUNDARY123
                 * Content-Disposition: form-data; name="username"
                 ...
                 * ------BOUNDARY123--
                 * 
                 * What Parsing Does: The MultipartParser takes this messy chunk and separates
                 ...
                 * 
                 * Without parsing, the server would just see one big confusing mess and
                 * wouldn't know
                 * where the photo starts, where the username is, or what the filename should
                 * be!
                 */
                ...
                /*
                 * will tmp file in C drive stores multiple users files when uploaded at the
                 * same time?
                 * ANS: System.getProperty("java.io.tmpdir") usually points to something like
                 * C:\Users\<username>\AppData\Local\Temp.
                 * 
                 * This means even if two users upload files with the same name at the same
                 * time, their files will have different names in the temp folder because of the
                 * random UUID prefix.
                 ...
                 * All files are stored in the same directory (peerlink-uploads inside the
                 * system temp directory), but their names are unique.
                 * 
                 * Suppose two users upload a file called photo.jpg at the same time:
                 * User 1’s file might be saved as:
                 * C:\Users\<username>\AppData\Local\Temp\peerlink-uploads\c1a2b3c4-5678-1234-
                 * 9abc-def012345678_photo.jpg
                 * User 2’s file might be saved as:
                 * C:\Users\<username>\AppData\Local\Temp\peerlink-uploads\9f8e7d6c-5432-4321-
                 * 8fed-cba987654321_photo.jpg
                 */
                ...
                */
                 ...
                 * transformed into a invite code for the uploader to send to the one who is
                 * going to dowload the file
                 */
                try (OutputStream os = exchange.getResponseBody()) {
    ...
    private class DownloadHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            String path = exchange.getRequestURI().getPath(); // the api path is URI
            // path looks like "/download/8192" 8192 is the port number
            ...
            try {
                ...
                try (Socket socket = new Socket("localhost", port);
                     ...
                     * 
                     * InputStream socketInput = socket.getInputStream(); ->
                     * This gets the input stream from the socket, which lets you read data sent
                     * from the
                     ...
                     * 
                     * File tempFile = ...:->
                     * This saves the reference to the temporary file so you can write to it and use
                     * it later.
                     */
