...
export default function InviteCode({ port }: InviteCodeProps) {
  ...
  return (
    <div className="mt-4 p-6 bg-[#1a1a1a]/40 backdrop-blur-md border border-[#3c3c3c] rounded-2xl">
      ...
      <div className="flex items-center h-[48px] rounded-xl overflow-hidden border border-[#3c3c3c]">
        <div className="flex-1 h-full bg-[#0d0d0d]/40 backdrop-blur-md px-4 flex items-center border-r border-[#3c3c3c] font-mono text-[20px] text-white">
          {port}
        </div>
        <button
          onClick={copyToClipboard}
          className="h-full px-6 bg-transparent text-white hover:bg-white hover:text-black transition-colors flex items-center justify-center"
